 ...
 * to verify thread safety of the C ABI and sink dispatchers.
 */
int main(int argc, char* argv[]) {
    const char* log_file = (argc > 1) ? argv[1] : nullptr;

    // Use DEBUG for console if specified, otherwise keep console quiet during high-volume test
    ...
    if (status != LOGGER_OK) {
        std::cerr << "Failed to initialize logger in test\n";
        return 1;
    }
